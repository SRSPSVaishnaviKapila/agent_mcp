"""Local PowerPoint backend used when MCP stdio is unavailable — rich themed slides."""
...
SLIDE_W = None  # set after import
SLIDE_H = None


def _init_dims():
    global SLIDE_W, SLIDE_H
    if SLIDE_W is None and Inches is not None:
...
class LocalPPTBackend:
    ...
    def _ensure_supported() -> None:
        if Presentation is None:
            raise RuntimeError(
                "python-pptx is not stable on Python 3.14. Use Python 3.11 or 3.12."
            )

    @staticmethod
    def list_tools() -> list[str]:
        return [
            "create_presentation", "plan_slides", "generate_content",
    ...
    @classmethod
    def _call_hf_text_model(cls, prompt: str) -> str | None:
        if sys.version_info >= (3, 14):
            return None
        try:
            import requests
        ...
        try:
            r = requests.post(url, headers=cls._hf_headers(), json=payload, timeout=60)
            r.raise_for_status()
            data = r.json()
        except Exception:
            return None
    ...
    @classmethod
    def _call_hf_image_model(cls, prompt: str) -> bytes | None:
        if sys.version_info >= (3, 14):
            return None
        try:
            import requests
        ...
        url = f"https://api-inference.huggingface.co/models/{model}"
        try:
            r = requests.post(url, headers=cls._hf_headers(),
    ...
    def _build_title_slide(self, slide, title: str):
        p = self.palette
    ...
    def _build_layout_a(self, slide, title: str, bullets: list[str]):
        """Light bg · dark left panel · color title strip · bullet dots."""
        p = self.palette
        ...
        nr.text = f"{self.slide_index:02d}"; nr.font.name = "Georgia"; nr.font.size = Pt(12)
    ...
    def _build_layout_b(self, slide, title: str, bullets: list[str]):
        """Split: light left content · dark right panel with decorative rings."""
        p = self.palette
        _add_rect(slide, 0, 0, SLIDE_W, SLIDE_H, p["bg_light"])
        _add_rect(slide, Inches(8.0), 0, Inches(5.333), SLIDE_H, p["bg_dark"])
        # Decorative rings on right panel
        ...
        circ2.fill.solid(); circ2.fill.fore_color.rgb = _rgb(p["bg_dark"]); circ2.line.fill.background()
        _add_text_box(slide, f"{self.slide_index:02d}", Inches(9.8), Inches(5.8), Inches(2),
                      Inches(0.6), "Georgia", 28, True, p["accent"], PP_ALIGN.CENTER)
        # Top accent bar (left panel only)
    ...
    def _build_layout_c(self, slide, title: str, bullets: list[str]):
        """Dark bg · numbered feature cards in 2–3 column grid."""
        p = self.palette
        _add_rect(slide, 0, 0, SLIDE_W, SLIDE_H, p["bg_dark"])
    ...
    def _build_conclusion_slide(self, slide, title: str, bullets: list[str]):
        """Dark centred conclusion slide with hollow ring decorations."""
        p = self.palette
        _add_rect(slide, 0, 0, SLIDE_W, SLIDE_H, p["bg_dark"])
    ...
    def _build_image_slide(self, slide, title: str, image_bytes: bytes | None, caption: str = ""):
        """Full-bleed image slide with title overlay bar."""
        p = self.palette
        _add_rect(slide, 0, 0, SLIDE_W, SLIDE_H, p["bg_dark"])
        if image_bytes:
            stream = self._fit_image(image_bytes)
    ...
    def add_slide(self, title: str, bullets: list[str] | None = None) -> str:
        ...
        if self.slide_index == 1:
            self._build_title_slide(slide, title)
        elif title.lower() in conclusion_titles:
            self._build_conclusion_slide(slide, title, normalized)
        else:
            layout = self.slide_index % 3
            if layout == 0:
                self._build_layout_a(slide, title, normalized)
            elif layout == 1:
                self._build_layout_b(slide, title, normalized)
            else:
                self._build_layout_c(slide, title, normalized)

        self.slides_added += 1
        return f"Added slide: {title}"

    def add_image_slide(self, title: str, image_prompt: str = "", caption: str = "") -> str:
        prs = self._require_presentation()
        self.slide_index += 1
        slide = prs.slides.add_slide(prs.slide_layouts[6])
        prompt = image_prompt.strip() or f"High quality educational illustration of {title}"
        image_bytes = self._call_hf_image_model(prompt)
        self._build_image_slide(slide, title, image_bytes, caption)
        self.slides_added += 1
        status = "AI-generated" if image_bytes else "placeholder"
        return f"Added image slide: {title} ({status})"

    def save_presentation(self, filename: str = "output.pptx") -> str:
        prs = self._require_presentation()
        path = self._output_path(filename)
        prs.save(path)
        return f"Saved presentation: {path.name}"

    def plan_slides(self, topic: str, num_slides: int = 5) -> str:
        ...
        generated = self._call_hf_text_model(prompt)
        parsed = self._extract_json(generated) if generated else None
            ...
            for item in parsed:
                if isinstance(item, dict) and "title" in item:
                    cleaned.append({
                        ...
                        "bullets": self._normalize_bullets(item.get("bullets", [])),
                    })
    ...
    def generate_content(self, title: str) -> str:
        prompt = (
        ...
        fallback = [
            f"What {title} is",
            f"Why {title} matters",
            f"Key components of {title}",
            f"Practical examples of {title}",
        ]
